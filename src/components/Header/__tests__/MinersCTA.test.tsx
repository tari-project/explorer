...
vi.mock('@number-flow/react', () => ({
  default: ({ value, format }: { value: number; format: any }) => (
...
describe('MinersCTA', () => {
  ...
  it('should open download link when button is clicked', () => {
    ...
    expect(mockOpen).toHaveBeenCalledWith('https://windows-download.com', '_blank')
  })
  ...
  it('should handle different OS download links', async () => {
    ...
    fireEvent.click(downloadButton)
    expect(mockOpen).toHaveBeenCalledWith('https://linux-download.com', '_blank')

    // Test Unknown OS
    getOS.mockReturnValue('Unknown')
    rerender(
      <TestWrapper>
        <MinersCTA theme="light" />
      </TestWrapper>
    ...
    downloadButton = screen.getByText('Download Tari Universe')
    fireEvent.click(downloadButton)
    expect(mockOpen).toHaveBeenCalledWith('https://default-download.com', '_blank')
  })
