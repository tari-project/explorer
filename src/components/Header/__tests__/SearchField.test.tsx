...
describe('SearchField', () => {
  ...
  it('should update query when user types in input', () => {
    render(
      <TestWrapper>
        <SearchField isExpanded={true} setIsExpanded={mockSetIsExpanded} />
      </TestWrapper>
  ...
  it('should navigate to block page when valid hash is entered', () => {
  ...
  it('should close search field when close button is clicked', () => {
    ...
    // Click any button (which would be the close button when empty)
  ...
  it('should clear input after successful search', () => {
    ...
    const searchInput = screen.getByLabelText('Search by height or hash') as HTMLInputElement
    fireEvent.change(searchInput, { target: { value: '12345' } })
    fireEvent.keyPress(searchInput, { key: 'Enter', code: 'Enter', charCode: 13 })
