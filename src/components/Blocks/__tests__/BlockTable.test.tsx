...
vi.mock('@services/api/hooks/useBlocks', () => ({
  ...
  useGetBlocksByParam: (firstHeight: number, blocksPerPage: number) =>
    mockUseGetBlocksByParam(firstHeight, blocksPerPage),
}));
...
vi.mock('../SkeletonLoader', () => ({
  default: ({ height }: { height: number }) => (
    <div data-testid="skeleton-loader" style={{ height }}>
      Loading skeleton
    </div>
  ),
}));
...
describe('BlockTable', () => {
  ...
  it('should render links to individual block pages', () => {
    ...
    const heightLinks = blockLinks.filter(
      (link) =>
