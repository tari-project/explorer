...
describe('GridItem', () => {
  ...
  it('should render with copy functionality when copy is true', () => {
    ...
    expect(screen.getByTestId('copy-button')).toHaveAttribute('data-copy', 'abcdef123456789')
  })
  ...
  it('should handle empty or null values', () => {
    const nullResult = GridItem('Empty', null, false, 0, 0, false)
    const undefinedResult = GridItem('Undefined', undefined, false, 0, 0, false)
  ...
  it('should apply correct grid layout', () => {
    ...
    const labelGrid = container.querySelector('[class*="MuiGrid-grid-xs-12"][class*="MuiGrid-grid-md-4"]')
    expect(labelGrid).toBeInTheDocument()

    // Check value grid item has correct sizes
    const valueGrid = container.querySelector('[class*="MuiGrid-grid-xs-12"][class*="MuiGrid-grid-md-8"]')
